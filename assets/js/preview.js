...
if(!client)fail("Supabase chưa được cấu hình.");else{
  ...
  if(!session)fail("Bạn cần đăng nhập admin trước.");else{
    ...
    if(!Number.isSafeInteger(id)||id<1)fail("ID hồ sơ không hợp lệ.");else{
      ...
      if(error||!data)fail("Session không đủ quyền hoặc hồ sơ không tồn tại.");else if(!/^case-00[12]$/.test(data.slug))fail("Hồ sơ này chưa có renderer preview.");else{frame.src=`cases/${data.slug}.html`;frame.addEventListener("load",()=>{status.hidden=true;wrapper.hidden=false},{once:true})}
    }
  }
}
